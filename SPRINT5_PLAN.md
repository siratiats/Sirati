# Sprint 5 — plan and backlog state

Prepared 2026-09-08 after the Sprint 4 sign-off. Theme: **make it shippable.**

Everything below is live in Jira. Sprint 5 candidates carry the label `sprint-5`:

```
project = SIRATI AND labels = "sprint-5" ORDER BY priority DESC
```

**One thing I could not do from here:** board 36 has only *SIRATI Sprint 1* (id 43, closed) — sprints 2, 3 and 4 were never created in Jira, so those sprints ran in name only. There is no sprint-creation API in the tools available to me, so create *SIRATI Sprint 5* in the board UI and pull in the `sprint-5` items with the JQL above.
...
## Backlog — filed, deliberately not in Sprint 5
